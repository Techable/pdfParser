    ...
    def update_pending_shareholder_type(self, list_of_t, page_values, parser_obj):
        ...
        if len(list_of_t) == 1:
            ...
            if list_of_t[0].text in expected_headings:
                index = self.get_index(list_of_t[0].y, 99.73, [48.40, 24.34, 95.73], page_values)
                ...
                #print "YYYY temp_page_values", temp_page_values
                ...
                if index in page_values and parser_obj.shareholder_type:
                    ...
                    if 1:
                        while(shareholder_type_table_fields in [6, 5, 2]):
                            ...
                            if(shareholder_type_table_fields in [6,5]):
                            ...
                            if(shareholder_type_table_fields == 2):
                                    if "Ordinary(Number)" in temp_page_values[index][0].text:
                                        index = round(index - 27, 2)
                                        shareholder_type_dict['ordinary_num'] = temp_page_values[index][0].text
                                        shareholder_type_dict['currency'] = temp_page_values[index][1].text
                                        ordinary_num = temp_page_values[index][0].text
                                        currency = temp_page_values[index][1].text
                                        parser_obj.ordinary_num = ordinary_num
                                        parser_obj.currency = currency
                            index = self.get_index(index, 81, [24, 25, 47, 35, 58], temp_page_values)
